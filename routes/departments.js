...
router.post('/', (req, res) => {
    ...
    {
        return res.status(400).json({ error: 'All fields are required: DepartmentID, DepartmentName, Location'});
    }
...
router.get('/:DepartmentID', (req, res) => {
    ...
    db.query('SELECT * FROM Department WHERE DepartmentID = ?', [DepartmentID], (err, results) => {
...
router.put('/:DepartmentID', (req, res) => {
    ...
    db.query(
        'UPDATE Department SET DepartmentName = ?, Location = ?',
