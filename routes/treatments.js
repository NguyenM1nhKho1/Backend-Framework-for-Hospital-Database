...
router.post('/', (req, res) => {
    ...
    {
        return res.status(500).json({ error: 'TreatmentID, TreatmentName and Cost are required' });
    }
...
router.get('/:TreatmentID', (req, res) => {
    ...
    db.query('SELECT * FROM Treatment WHERE TreatmentID = ?', [TreatmentID], (err, results) => {
        if (err) return res.status(500).json({ error: 'Database error' });
        if (results.length === 0) return res.status(404).json({ error: 'Treatment not found' });
        res.json(results[0]);
    });
})
...
router.put('/:TreatmentID', (req, res) => {
    ...
    db.query(
        'UPDATE Treatment SET TreatmentName = ?, Description = ?, Cost = ?'
        [TreatmentID, Description || null, Cost],
        (err, results) => {
            if (err)
                {
                    console.error(err);
            ...
            if (results.affectedRows === 0) return res.status(404).json({ error: 'Treatment not found' });
