...
// Add a new doctor
router.post("/", (req, res) => {
  const { DoctorID, Name, Specialization, PhoneNumber, DepartmentID } =
    req.body;
...
router.put("/:DoctorID", (req, res) => {
  ...
  db.query(
    "UPDATE Doctor SET Name = ?, Specialization = ?, PhoneNumber = ?, DepartmentID = ? WHERE DoctorID = ?",
    [Name, Specialization, PhoneNumber, DepartmentID, DoctorID],
    (err, results) => {
      if (err) {
        console.error(err);
        return res.status(500).json({ error: "Database error" });
